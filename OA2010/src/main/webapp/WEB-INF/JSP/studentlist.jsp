    ...
    <div class="layui-row" style="margin-top: 10px">
       <form action="/student_list/1/${pageUtils.pageSize}" method="post" id="ff">
        ...
        <div class="layui-col-xs2">
            <div class="layui-form-item">
                <div class="layui-input-block">
                    <a class="layui-btn layui-btn-mini layui-btn-mini" onclick="toExcel()" href="javascript:alert('导出中...');" lay-event="detail">导出Excel</a>
                </div>
            </div>
        </div>
    </div>
...
<div class="layui-container">
    <table class="layui-table" id="tbdata" lay-filter="tbop">
        ...
        <tr>
            <td>${student.id}</td>
            <td>${student.name}</td>
            <td>${student.tClass.className}</td>
            <td>${student.sex}</td>
            <td>${student.phone}</td>
            <td>${student.email}</td>
            <td>
                <shiro:hasPermission name="student:update">
                <a class="layui-btn layui-btn-mini" href="studentupdate.html">编辑</a>
                </shiro:hasPermission>
                <shiro:hasPermission name="student:skim">
    ...
    $(function () {
        $.getJSON("/student_getClasses",null,function(arr){
            ...
            for(i=0;i<arr.length;i++){
                if(arr[i].id== "${requestScope.classId}"){
